...
# initialize timers for msg PRINT , msg tg ,buy limit , msg TGV toaster(notif windows)
...
#
    ...
    # Swaping
        ...
        pancakeswap2_txn = contract.functions.swapExactTokensForETH(amount,
                    0,
                    [The_Token, token_bnb],mywallet,(int(time()) +4)  #deadline in secondes
...
#
        ...
        #-------------
                lqtc.Swap(quantiter)
        #-------------
                BuyedTime = time()            # get time before and after swap, to see the timing g
                message = (f' {quantiter} {lqtc.NomToken} SWAPED at {lqtc.prix:.5f}\n pcs-Time: {lqtc.TimeView}\n Buying-Epoch:{BuyingTime}\n Buyed-Epoch: {BuyedTime}')
...
#
# send a TG message every x sec if liquidy +10000
#
            if (lqtc.liqui >= 10000) and ((timeactu - lqtc.timebaseTGV) >=20):    # msg every x SECS if liquidy + than
                message =(f' {lqtc.NomToken} Liquidity: {lqtc.liqui:.2f}  price: {lqtc.prix:.5f} pcs-Time: {lqtc.TimeView}')
                lqtc.Send_TG(message)
                print(message)
                lqtc.timebaseTGV = timeactu      # set actual time as ref at next loop
#
# msg Tg every 180 minutes (10800)
#
...
#
            if (timeactu - lqtc.timebasePRT) >=1800:     # msg in local every x minutes
